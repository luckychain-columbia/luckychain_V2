        ...
        formData.maxEntrants.trim().length > 0
          ? (() => {
              ...
              if (parsed < numWinners) {
                throw new Error(
                  `Max entrants (${parsed}) cannot be less than number of winners (${numWinners})`
                );
              }
              if (parsed > 10000) {
      ...
      <DialogContent className="bg-black/80 backdrop-blur-xl border-white/20 sm:max-w-[900px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription className="text-gray-300 text-base">
            Set up your raffle parameters. All fields are stored on the
            blockchain.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-6 mt-6">
          ...
          <div className="space-y-2">
            ...
            />
            <p className="text-sm text-gray-400">
              How much ETH to enter the raffle
            </p>
          </div>
            ...
            <div className="flex items-center gap-4">
              <input
                ...
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    creatorFee: Number(e.target.value),
                  })
                }
                className="flex-1 h-2 bg-white/20 rounded-lg appearance-none cursor-pointer accent-purple-500"
              />
              <span className="text-2xl font-bold text-purple-400 min-w-[80px] text-right">
                {formData.creatorFee}%
              </span>
            </div>
            <p className="text-sm text-gray-400">
              Creator keeps {formData.creatorFee}% of the total pot. Winners
              share {100 - formData.creatorFee}% evenly.
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            />
            <p className="text-sm text-gray-400">
              Prize pool will be split equally among winners (1-100)
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              onBlur={(e) => {
                ...
                const numValue = parseInt(value, 10);
                const numWinnersValue = parseInt(
                  formData.numberOfWinners.trim() || "1",
                  10
                );
                ...
                if (numValue < minWinners) {
                  ...
                  setFormData({
                    ...formData,
                    maxEntrants: minWinners.toString(),
                  });
                }
              }}
              className="bg-white/10 border-white/20 text-white placeholder:text-gray-400"
            />
            <p className="text-sm text-gray-400">
              Leave empty for unlimited participants. Must be at least{" "}
              {formData.numberOfWinners.trim() || "1"} (number of winners) and
          ...
          <div className="flex items-start space-x-3 p-4 rounded-lg border border-white/20 bg-white/10">
            <Checkbox
              ...
              onCheckedChange={(checked) =>
                setFormData({
                  ...formData,
                  allowMultipleEntries: checked as boolean,
                })
              }
              className="mt-1"
            />
            <div className="space-y-1">
              ...
              <p className="text-sm text-gray-400">
                If checked, users can buy multiple tickets for this raffle
              </p>
            </div>
          </div>
          ...
          <div className="space-y-3 pt-4">
            ...
            >
              {isLoading
                ? "Creating..."
                : formData.seedPrizePool &&
                  parseFloat(formData.seedPrizePool) > 0
                ? `Create Raffle & Seed ${parseFloat(
                    formData.seedPrizePool
                  ).toFixed(4)} ETH`
                : "Create Raffle"}
            </Button>
            <p className="text-sm text-gray-400 text-center">
              {formData.seedPrizePool && parseFloat(formData.seedPrizePool) > 0
                ? `This will create a transaction that includes ${parseFloat(
                    formData.seedPrizePool
                  ).toFixed(
                    4
                  )} ETH to seed the prize pool. Please approve in your wallet.`
